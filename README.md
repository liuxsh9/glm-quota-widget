...
- 多显示器以主显示器工作区为定位基准：拔屏只对被拔的那块屏派发 `display-removed`、插屏只派发 `display-added`（幸存屏度量没变时没有 `display-metrics-changed`），而且多屏事件是**错峰到达**的，所以三个事件统一走 300ms 防抖，落定后把窗口收回工作区；睡眠唤醒锁屏解锁后还会再自检两次（睡眠期间的插拔常常一个事件都收不到）。面板宽 326 固定，**高度按实测内容上报**（各页签取高者），所以切页签窗口一像素都不动
