...
- **API Key 配置，长期有效**：控制台 API Key 一配永逸；也可用登录 Cookie（约 3 天失效，挂件变红提醒 → 点击直达设置 → 粘贴新值，自动从整段 Cookie 里提取 token，检测到剪贴板有新 token 会提示一键填入）
...
2. **推荐**：登录 [bigmodel.cn](https://www.bigmodel.cn/coding-plan/personal/overview) → 控制台「API Keys」→ 创建/复制 API Key（长期有效，一劳永逸）
...
- API Key 长期有效；Cookie JWT 由服务端会话决定有效期（约 3 天），挂件靠接口返回判定失效并提醒
- 多显示器场景以主显示器工作区为定位基准
