 ...
 *   name        展示名（设置页分段标题、账户菜单）
 *   tab         短名（设置页「添加 X 账户」、默认账户名等）；面板页签用它
 *   tabShort    面板页签上更短的写法（选配）：页签那一行要塞「页签组 + 更新时间 + 两个按钮」，
 *               名字长的（DeepSeek）会把这个空间撑爆、把 ⚙ 挤出面板右边缘
...
(function (factory) {
  ...
  const list = [
    ...
    },
    {
      ...
      tab: 'DeepSeek',
      tabShort: 'DS',           // 页签窄，全名会把右边的 ⚙ 挤出去（设置页里仍是全名）
      accentMode: 'fixed',
